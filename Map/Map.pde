...
void draw() {
  if (startscreen) {
    ...
    text("About:", 5, 20);
    textFont(f, 16);
    text("About the game:", 5, 50);
    textFont(f, 12);
    text("Less Advanced Wars TBS is a turn-based strategy game in which two players compete to eliminate the other's units or capture the other's", 40, 70);
    text("headquarters.", 20, 82);
    textFont(f, 16);
    text("Gameplay:", 5, 112);
    textFont(f, 12);
    text("Each turn, players get money based on how many buildings they own in order to build units. ", 20, 132);
    textFont(f, 14);
    text("Units:", 20, 152);
    textFont(f, 12);
    text("There are three main types of units: Ground, air, and water. Units are can move and attack, and are used to destroy enemy units, or capture", 40, 172);
    text("buildings. Units may move and attack once per turn, but must move before they attack. Only a single unit may occupy a given space, but units", 20, 184);
    text("are allowed to pass through friendly units. In special cases,units may 'load into' other units for transport and then unload to attack.", 20, 196);
    text("Units may not attack or move on the turn they are built.", 20, 208);
    textFont(f, 14);
    text("Buildings:", 20, 228);
    textFont(f, 12);
    text("There are four types of buildings: Headquarters, bases, airports, seaports, and cities.", 40, 248);
    text("Headquarters: The main objective of the game -must be protected at all costs. Each player has only one. When it is captured, the game ends.", 20, 260);
    text("Base: Used to build ground units.", 20, 272);
    text("Airports: Used to build air units.", 20, 284);
    text("Seaports: Used to build sea units.", 20, 296);
    text("Cities: Can be captured for additional income.", 20, 308);
    text("As players take turns, they can capture more airports, bases, and seaports to increase their building capacity, and capture more cities to", 20, 328);
    text("increase their income. Eventually if a player becomes strong enough, he can move towards the enemy territory to end the game by destroying", 20, 340);
    text("all enemy units, or capturing the headquarters.", 20, 352);
    textFont(f, 16);
    text("About the project:", 5, 382);
    textFont(f, 12);
    text("This is the APCS2 final project of Team CQNNS (Chris Sher, Nick Ng, and Kevin 'Q' Hwang). This project incorporates data structures and", 20, 394);
    text("algorithms learned this term such as circularly linked lists for storing units and pathfinding algorithms for determining where a unit may move.", 20, 406);
    text("We hope that enjoy our game and memes.", 20, 418);
    fill(255, 255, 255);
    rect(20, 600, 100, 25);
    fill(0);
    textFont(f, 16);
    ...
    fill(0);
    textFont(f, 20);  
    text("How To Play: ", 5, 20);
    textFont(f, 14);
    text("- Use the arrow keys (Up, down, left, right) to move the cursor.", 20, 40);
    text("- If a unit is selected, use 'a' to begin the attack phase, or 'm' to begin the move phase.", 20, 60);
    text("- While in the attack phase, move the cursor to a square marked green and press 'x' to execute the attack.", 20, 80);
    text("- While in the movement phase, move the cursor to a square marked pink and press 'x' to execute the move.", 20, 100);
    text("- Using 'x' again on an unmarked square will deselect the unit and cancel the action.", 20, 120);
    text("- In the case of carrier units, a unit may be moved onto another unit. When this is done the moving unit will disappear", 20, 140);
    text("and the carrier unit will be marked with an 'L' to indicate that it is loaded.", 20, 160);
    text("- To unload the unit, simply press 'u' on a square next to a loaded unit and the unit will reappear there and the 'L' will be.", 20, 180);
    text("removed. 'i' will also unload. 'u' and 'i' unload the first and second slots respectively, if applicable.", 20, 200);
    text("- If a building is selected, if the building is an airport, base, or seaport, the player's mouse can be used to select units", 20, 220);
    text("to build from the menu on the right side of the screen.", 20, 240);
    text("- Moving the cursor away will take the menu away, and using 'x' again will deselect the building.", 20, 260);
    text("- To capture a building, move an infantry or mech unit over a building. Keeping it there without moving for two turns will", 20, 280);
    text("capture the building.", 20, 300);
    text("- After a turn has been completed, press 'ENTER' to begin the next person's turn.", 20, 320);
        fill(255, 255, 255);
    rect(20, 600, 100, 25);
    fill(0);
    textFont(f, 16);
...
void mouseClicked() {
  ...
  if (startscreen) {
    ...
    if (mouseX >= 20 && mouseX <=120 && mouseY >= 600 && mouseY <= 625) {
      startscreen = true;
      about = false;
    }
  } else if (info) {
        ...
        if (curr.selected.canTransport && curr.selected.transported2 != null) {
          if ((Math.abs(_board.xcor - curr.selected.x) == 16 && _board.ycor == curr.selected.y) || (Math.abs(_board.ycor - curr.selected.y) == 16 && _board.xcor == curr.selected.x)) {
            if (_board._board[_board.ycor/16][_board.xcor/16].terrain.footHindrance < 10 && _board._board[_board.ycor/16][_board.xcor/16]._unitG == null) {
              ...
              curr.selected.transported2.moved = true;
              curr.selected.moved = true;
...
void terrainInfo() {
  ...
  //text("Selected",642,18); 
  textFont(f, 16);
  image(_board._board[_board.ycor/16][_board.xcor/16].terrain.img, 800, 502); 
  text("Defense: " + _board._board[_board.ycor/16][_board.xcor/16].terrain.defense, 642, 527); 
  text("Foot Hindrance: " + _board._board[_board.ycor/16][_board.xcor/16].terrain.footHindrance, 642, 549);
  text("Wheel Hindrance: " + _board._board[_board.ycor/16][_board.xcor/16].terrain.wheelHindrance, 642, 574);
  text("Tread Hindrance: " + _board._board[_board.ycor/16][_board.xcor/16].terrain.treadHindrance, 642, 599);
